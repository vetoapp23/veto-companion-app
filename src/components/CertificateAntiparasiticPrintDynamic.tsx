...
export function CertificateAntiparasiticPrintDynamic({ animalId }: CertificateProps) {
  ...
  const handlePrint = async () => {
    ...
    const printWindow = window.open('', `antiparasitic_certificate_${animal.id}`, 'height=900,width=820');
    if (!printWindow) {
      setPrinting(false);
    ...
    try {
      ...
      printWindow.document.write(
        `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>${escapeHtml(t('antiparasiticCertificate.heading'))}</title></head><body style="font-family:Arial,sans-serif;padding:24px;color:#555;">${escapeHtml(t('alerts.printPreparing', { defaultValue: 'Preparing…' }))}</body></html>`
      );
      printWindow.document.close();

      const issuedAt = format(new Date(), 'PPp', { locale: dateFns });
      const qrPayload = [
        t('antiparasiticCertificate.heading'),
            ...
            <table class="doses-table">
              ...
              <tbody>
                ${doseRows
                  ...
                  <tr class="${row.status === 'planned' ? 'row-planned' : 'row-done'}">
                    ...
                    <td>
                      <span class="badge ${row.status === 'planned' ? 'badge-planned' : 'badge-done'}">
                        ${row.status === 'planned' ? e(t('antiparasiticCertificate.statusPlanned')) : e(t('antiparasiticCertificate.statusDone'))}
                      </span>
                    </td>
                  ...
                  ${
                    row.notes
                      ? `<tr class="notes-row"><td colspan="5"><em>${e(t('antiparasiticCertificate.notes'))}</em> ${e(row.notes)}</td></tr>`
                      : ''
                  }
                `
                  )
      ...
      const printContent = `
        ...
        <html>
          <head>
            ...
            <title>${e(t('antiparasiticCertificate.docTitle', { name: animal.name }))}</title>
            <style>
          ...
          <body>
            ...
            <div class="header">
              <div class="header-main">
                ...
                <h1>${e(t('antiparasiticCertificate.heading'))}</h1>
              </div>
  ...
  if (!animal || !client) {
    return (
      <Button variant="outline" disabled size="sm" className="w-full sm:w-auto justify-center">
        ...
        <span className="sm:hidden">{t('antiparasiticCertificate.buttonShort')}</span>
        <span className="hidden sm:inline">{t('antiparasiticCertificate.button')}</span>
      </Button>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2 w-full sm:w-auto">
      ...
      >
        <Printer className="h-4 w-4 mr-2 shrink-0" />
        <span className="sm:hidden">{t('antiparasiticCertificate.buttonShort')}{doseRows.length > 0 ? ` (${doseRows.length})` : ''}</span>
        <span className="hidden sm:inline">
          {t('antiparasiticCertificate.button')}{doseRows.length > 0 ? ` (${doseRows.length})` : ''}
        </span>
      </Button>
    </div>
  );
